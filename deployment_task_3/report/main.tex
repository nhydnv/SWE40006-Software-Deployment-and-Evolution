...
I launched a new EC2 instance called \texttt{Task 3.1}. This instance runs Amazon Linux 2023, uses a \texttt{t3.micro} instance type to remain eligible for the free tier, and is configured with the previously generated key pair.
...
Once I could connect to the database, I created a new database \texttt{wordpress\_db} to be used by my application. I then exited the DB connection, configured the \texttt{wg-config.php} file with the required database information, including the database name, username, and password, along with the other necessary credentials. This allowed the WordPress application to connect to and use the configured database. Finally, I restarted the Apache web server and verified that the application has been successfully configured to use the database. As I had not installed the actual WordPress application by this point, the database displayed all empty entries.
...
I encountered an issue where the WordPress application displayed the error "Error establishing a database connection", even though I had verified that the database credentials in the \texttt{wg-config.php} file were correct. I determined the the issue was caused by RDS being configured to require encrypted (SSL) connection, while WordPress was attempting to connect without encryption. As a result, RDS rejected the connection. This was also evident in the earlier command I used to connect to the database, where I had to include the flags \texttt{--ssl-verify-server-cert} and \texttt{--ssl-ca=/etc/pki/rds/global-bundle.pem}. These flags told the MySQL client to use SSL and verify the RDS server's SSL certificate using the specified certificate.

I resolved the issue by configuring WordPress to request an SSL connection by adding \texttt{define('MYSQL\_CLIENT\_FLAGS', MYSQLI\_CLIENT\_SSL);} to \texttt{wp-config.php}.
...
\vspace{-1em}
\caption{Creating a new S3 bucket.}
\end{figure}
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.4]{images/2_server_rmed.jpg}
\vspace{-1em}
...
The RDS instance's security group was only allowing MySQL traffic on port 3306 from my original EC2 instance instead of the security group that was attached to all instances. This was fixed by adding the correct inbound security rule.
...
\begin{figure}[H]
\centering
\includegraphics[scale=0.4]{images/4_config_metric.jpg}
\vspace{-1em}
...
Overall, the implementation of this deployment task was relatively straightforward. The main challenges I encountered involved resolving the database connection issue in Task 3.2 and diagnosing the unhealthy instances in Task 3.3.

Troubleshooting the database connection was relatively simple because the root cause was easier to identify, and the solution only required an additional line of code. Meanwhile, resolving the unhealthy instances was much more challenging because there were so many potential causes. This meant that I had to verify several parts of the infrastructure and rebuild several components multiple times to ensure that everything was configured correctly before narrowing the problem down to a misconfigured security group rule. Eventually, I was able to identify the root cause and resolve the issue succesfully.
